...
async function testStoreFunctions() {
  ...
  try {
    ...
    // 현재 프로필 이름 가져오기
    const currentProfileName = await askar.storeGetProfileName({ storeHandle: storeHandle.handle });
    ...
    const profiles = await askar.storeListProfiles({ storeHandle: storeHandle.handle });
    ...
    // 변경 후 프로필 목록 다시 조회
    const profilesAfterRename = await askar.storeListProfiles({ storeHandle: storeHandle.handle });
    ...
    // 기본 프로필 조회
    const defaultProfile = await askar.storeGetDefaultProfile({ storeHandle: storeHandle.handle });
    console.log('기본 프로필:', defaultProfile);
    const deleteName = 'new-test-profile'
    // 프로필 삭제
    console.log(`프로필 삭제: ${deleteName}`);
    ...
    // 삭제 후 프로필 목록 조회
    const profilesAfterDelete = await askar.storeListProfiles({ storeHandle: storeHandle.handle });
    ...
    const removeStoreResult = await askar.storeRemove({ specUri: 'sqlite://:memory:' });
